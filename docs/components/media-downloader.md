...
- media updates попадают в heavy lane; число одновременных yt-dlp/ffmpeg ограничено `WORKER_HEAVY_CONCURRENCY`;
- abort worker-задачи завершает дочерний процесс через `SIGKILL` и очищает временную директорию;
