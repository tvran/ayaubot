...
`ticketon_chat_preferences` хранит общие настройки чата: `earliest_session_minute` в диапазоне 0…1439, `adjacent_seats` в диапазоне 1…6, автора и время последнего изменения. Время `0` означает отсутствие временного фильтра. Service отбрасывает более ранние локальные сеансы до запроса карты зала и ищет свободный блок выбранного размера.
