...
Файл `src/bot/app.js` связывает Telegram Bot API, Redis, рендереры цитат, фото-стикеров и демотиваторов, извлечение кадра видеокружка, загрузчик внешних видео, сервис аналитики, Percent Game, дни рождения и монитор kino.kz. Чистая логика отбора и разметки упоминаний находится в `src/bot/mentions.js`, проверка входа `/demotivation` — в `src/demotivation/service.js`, а выбор reply-фото для `/qs` — в `src/sticker/service.js`.
...
// → { api, chatAllowed, handleUpdate, notifyKinoAvailability }
```
...
- `notifyKinoAvailability(alert)` добавляет к тексту мониторинга те же проверенные упоминания, что использует `/all`.
...
Тот же внутренний helper принимает произвольный header для автоматического оповещения kino.kz. Если список известных участников недоступен или пуст, важный текст о местах всё равно отправляется без mention entities.
