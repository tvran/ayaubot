...
Redis и PostgreSQL технически необязательны и включаются независимо. Без Redis команды цитирования не смогут собрать сообщения, а `/percent` не станет выдавать несохраняемый результат. Без PostgreSQL аналитические команды вернут служебный текст, а обычные сообщения не будут сохраняться.
