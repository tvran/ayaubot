...
- PostgreSQL для аналитики и игр;
...
| `test` | `node --test` | Автоматические тесты, сейчас для Media Service |
...
| `DATABASE_URL` | Для аналитики | PostgreSQL connection string; при отсутствии DB adapter равен `null` |
...
Файл `.env.example` содержит основной минимум и настройки media-сервиса, но в текущей версии не перечисляет `DATABASE_URL`, PostgreSQL-настройки, `APP_URL`, `WEBHOOK_PATH`, `PORT` и множественный `ALLOWED_CHAT_IDS`.
