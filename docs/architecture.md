...
6. `/q`, `/qs` и `/qd` направляются в подсистему цитат; команды дней рождения — в Birthday Service; остальные известные команды — в аналитику.
...
- Автоматические тесты покрывают URL-фильтр, базовый lifecycle Media Service, разбор дат и idempotency планировщика дней рождения. `npm run lint` выполняет только синтаксическую проверку файлов.
