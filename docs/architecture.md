...
Worker раз в час под lease `ticketon-cinema-seat-monitor` проверяет, наступил ли локальный час утреннего дайджеста. Один раз в день он загружает будущие сеансы выбранных фильмов, применяет фильтры кинотеатра и минимального локального времени, затем читает публичные static/dynamic карты каждого оставшегося зала. Сервис объединяет ряды всех ценовых секторов для расчёта верхней половины целого зала, соединяет геометрию с наличием по ID и ищет самый большой последовательный блок не меньше выбранного чатом порога. Варианты одного сообщения группируются по фильму, внутри — по кинотеатру и затем по времени; минимальная цена сеанса выше 5000 ₸ получает индикатор дорогого билета. `(chat_id, digest_date)` атомарно фиксируется в `ticketon_daily_digests`. Сообщение не упоминает участников чата.
...
- PostgreSQL хранит известные профили для `/all` и аналитики, агрегаты слов, игры, ежедневные выборы, дни рождения, Ticketon-подписки и markers отправленных уведомлений. Старые birthday и Ticketon daily markers очищаются через 400 дней.
