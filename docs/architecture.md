...
Worker раз в час под lease `ticketon-cinema-seat-monitor` проверяет, наступил ли локальный час утреннего дайджеста. Один раз в день он загружает будущие сеансы выбранных фильмов, применяет фильтры кинотеатра и минимального локального времени, затем читает публичные static/dynamic карты каждого оставшегося зала. Сервис объединяет ряды всех ценовых секторов для расчёта верхней половины целого зала, соединяет геометрию с наличием по ID, ищет выбранное чатом число последовательных мест и собирает все варианты в одно сообщение. `(chat_id, digest_date)` атомарно фиксируется в `ticketon_daily_digests`; текст передаётся общему механизму `/all`, поэтому к нему добавляются проверенные участники чата.
