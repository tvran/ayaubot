...
6. `/q`, `/qs` и `/qd` направляются в подсистему цитат; `/percent` — в Percent Game; команды дней рождения — в Birthday Service; остальные известные команды — в аналитику.
...
- PostgreSQL хранит профили, агрегаты слов, игры, ежедневные выборы, дни рождения и markers отправленных уведомлений. Старые birthday markers очищаются через 400 дней.
...
Отсутствие Redis не мешает аналитике, но `/q` не сможет получить даже ответное сообщение, а `/percent` вернёт сообщение о недоступном суточном хранении. Отсутствие PostgreSQL отключает хранение данных; сервис возвращает пользователю сообщение о ненастроенной аналитике.
