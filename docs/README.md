...
AyauBot — Telegram-бот для групповых чатов. Он превращает сообщения в статические WebP-стикеры и демотиваторы, скачивает Instagram Reels и TikTok-видео, зовёт известных участников чата, ведёт статистику слов, запускает игры, напоминает о днях рождения, мониторит хорошие места на kino.kz и ежедневно выбирает случайного участника чата.
...
| [Монитор kino.kz](components/kino-monitor.md) | Inline-выбор фильмов/кинотеатров, карты мест и часовой scheduler |
...
| [Рендерер демотиваторов](components/demotivation-renderer.md) | Reply-изображения, растяжение до рамки и подпись |
...
| `/kino` | Выбрать фильмы и кинотеатры для ежечасного мониторинга соседних мест |
...
| `/percent <параметр>` | Показать процент для автора команды или автора reply; результат действует 24 часа |
...
Команды допускают суффикс имени бота, например `/q@my_bot`. Аналитические функции и `/all` доступны только при настроенном PostgreSQL; создание многострочных цитат и суточные результаты `/percent` требуют Redis. Telegram Bot API не выдаёт боту полный список обычных участников, поэтому `/all` использует сохранённых авторов сообщений и команд, события вступления и текущих администраторов. Чтобы Telegram гарантированно разрешал проверять актуальный статус остальных пользователей, бот должен быть администратором чата.
...
src/kino/                    каталоги kino.kz, меню, карты мест и scheduler
...
config/percent-game.json     параметры и фразы процентной игры
