...
AyauBot — Telegram-бот для групповых чатов. Он превращает сообщения в статические WebP-стикеры, скачивает Instagram Reels и TikTok-видео, ведёт статистику слов, запускает игры, напоминает о днях рождения и ежедневно выбирает случайного участника чата.
...
Команды допускают суффикс имени бота, например `/q@my_bot`. Аналитические функции доступны только при настроенном PostgreSQL; создание многострочных цитат и суточные результаты `/percent` требуют Redis.
